...
class IFrontEnd : public TopLevel<IFrontEnd> {
 ...
 public:
  ...
  // max_useful bounds the caller's usable answer so implementations can stop
  // simulating ahead early; returning more than max_useful is allowed.
  virtual Clk_t idle_ticks(Clk_t max_useful) {
    return 0;
  }
  ...
  /**
   * @brief Receives an externally assigned, retry-stable request identity.
   *
   * @details frontend_id identifies the logical request and
   * frontend_sub_id distinguishes multiple memory transactions generated by
   * that request. Callers must pass the same pair on every backpressure retry;
   * a negative frontend_id explicitly marks a request as ineligible for stable
