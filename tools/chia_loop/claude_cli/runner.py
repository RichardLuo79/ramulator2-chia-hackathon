"""Independent Fable-effort CHIA runs. Preparation and tests are nonbillable."""
...
POLICY = {"version": "claude_cli_individual_run_v1", "model": T.MODEL, "usage_schema": U.SCHEMA,
          ...
          "promotion": "strict_two_objective_Pareto", "native_claude_tools": False,
...
def load_config(root):
    return R.read_json(pathlib.Path(root) / "claude_config.json")
...
def prepare(args):
    """Full-window, non-generating preparation; never starts a paid run."""
    limits = T.limits(args.max_iterations, args.usd_cap, args.cpus,
    ...
    with R.cpu_lease(REPO / "eval_out/chia/.cpu_leases", limits["cpu_budget"]):
        ...
        atomic_write_json(root / "claude_config.json", config)
        T.Ledger(root).transaction(lambda data: None)
        ...
        shutil.copyfile(binary, root / "runtime/claude")
        (root / "runtime/claude").chmod(0o755)
        ...
        pinned += [root / "seed/atomic_controller.cpp", root / "seed/candidate.so", root / "runtime/claude",
                   root / "runtime/libramulator.so", root / "runtime/isolated_sim"]
        atomic_write_json(root / "run_manifest.json", {"run_id": root.name, "record_type": "claude_optimization_run",
            ...
            "human_intervention": False, "human_operational_intervention": False,
            "human_modeling_hints": False, "prior_design_exposed": False, "archives_verified": True,
            "scientific_history_imported": False, "evaluation_interpretation": "fresh trial on previously used benchmark families",
...
def call(root, operation, system, conversation, role="proposal"):
    ...
    for attempt in range(3):
        ...
        try:
            result = get(generate.chia_remote(str(root), operation, system, conversation, role))
            # The CLI is reaped and the provider receipt settled before packing.
            # Keep long-running campaigns compact, not just at final completion.
            artifacts.compress(root, root / "interaction_archives.json", min_bytes=16384, parts={"interactions"})
            return result
...
def review(root, source, proposal, directory, operation):
    ...
    result = {**decision, **binding, "approved": verdict == "pass", "human_intervention": False,
              "reviewer_kind": "isolated_claude_cli", "time": time.time()}
...
def evolve_one(root, state, parent_id, iteration):
    policy = configured_policy(root)
    directory = root / "candidates" / f"fable_{iteration:03d}"
        ...
        for turn in range(saved["turn"], policy["model_turns_per_proposal"] + 1):
            ...
            if status == "inspect":
                ...
                if number > policy["drafts_per_iteration"]:
                    ...
                    break
                label = f"fable_{iteration:03d}_d{number:02d}"
...
def search(root):
    ...
    for iteration in range(len(state["history"]) + 1, stop + 1):
        ...
        if result["status"] == "evaluated":
            candidate_id = f"fable_{iteration:03d}"
...
def run(root, resume=False):
    ...
    with R.exclusive_lock(root / ".runner.lock"), R.cpu_lease(REPO / "eval_out/chia/.cpu_leases", config["cpu_budget"]):
        ...
        try:
            ...
            atomic_write_json(root / "run_manifest.json", manifest)
            E.verify_run_archives(root)
            if U.report(root)["audit_issues"]:
                raise RuntimeError("final LLM usage evidence audit failed")
            if (root / "interaction_archives.json").exists():
                artifacts.verify(root / "interaction_archives.json")
...
def supervise(root, resume=False):
    ...
    with R.exclusive_lock(root / ".supervisor.lock"):
        ...
        while time.time() - state["started_at"] < POLICY["maximum_run_wall_seconds"]:
            T.check_stop(root)
            verify(root)
            args = [sys.executable, "-m", "tools.chia_loop.claude_cli", "run", "--root", str(root), "--authorize-paid"]
...
def main():
    ...
    prep = commands.add_parser("prepare", help="non-generating full-window preparation; fresh root only")
    ...
    prep.add_argument("--cpus", type=int, default=3)
    ...
    for name in ("usage", "retrospective"):
        ...
        sub.add_argument("--root", required=True, type=pathlib.Path)
        sub.add_argument("--write", action="store_true")
    for name in ("run", "supervise"):
        sub = commands.add_parser(name)
        sub.add_argument("--root", required=True, type=pathlib.Path)
        sub.add_argument("--authorize-paid", required=True, action="store_true")
        sub.add_argument("--resume", action="store_true")
    args = parser.parse_args()
    try:
        if args.command == "prepare":
            prepare_with_wait(args)
        elif args.command in ("usage", "retrospective"):
            reporting = U if args.command == "usage" else V
            payload = reporting.write_report(args.root) if args.write else reporting.report(args.root)
