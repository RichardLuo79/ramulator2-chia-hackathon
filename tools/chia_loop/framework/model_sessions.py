...
from .config import ClaudeBackend, CodexBackend, VertexBackend
...
def upstream_identity() -> dict:
    ...
    from chia.models import claude, codex, vertex
...
class ModelSession:
    ...
    backend: CodexBackend | ClaudeBackend | VertexBackend
    session_id: str
    model: object
    settings: dict
    ...
    def checkpoint(self, result) -> NativeCheckpoint:
        ...
        if kind == "codex_cli":
            ...
            if result.session_state is None:
                raise ValueError("Vertex returned no native continuation")
            state = {"sdk": "google.genai.types.Content"}
...
def create_session(
    ...
    *,
    ...
    """Translate settings, using one common turn contract for all three backends.
    ...
    if not all(hasattr(cls, "prompt_once") for cls in (CodexLLM, ClaudeCodeLLM, VertexGeminiLLM)):
        raise RuntimeError(
            "install the pinned CHIA session extension before binding a native adapter"
        )
    dependency = upstream_identity()
    if not isinstance(backend, (CodexBackend, ClaudeBackend, VertexBackend)):
