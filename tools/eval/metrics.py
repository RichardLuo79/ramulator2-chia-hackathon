...
def validate_request_payload(payload, *, request_trace_scope,
                             request_trace_timebase, signatures=None,
    ...
    oracle_mean = finite_float(
        payload.get("oracle_read_mean_latency"), "oracle_read_mean_latency")
    if oracle_mean <= 0:
    ...
    eligibility_o = request_coverage(
        payload.get("stable_eligibility_fraction_o"),
        "stable_eligibility_fraction_o")
    eligibility_m = request_coverage(
        payload.get("stable_eligibility_fraction_m"),
    ...
    _require_ratio(eligibility_o, eligible_o, n_oracle,
                   "stable_eligibility_fraction_o")
    _require_ratio(eligibility_m, eligible_m, n_model,
                   "stable_eligibility_fraction_m")
    return payload


def _request_cache_is_current(payload, request_path, signatures, *,
                              request_trace_scope, request_trace_timebase,
                              expected_matcher_schema=None):
    if expected_matcher_schema is None:
        expected_matcher_schema = matcher_schema_version()
    try:
...
def load_or_compute_request(request_path, trace_path, oracle_trace_path, *,
                            force=False, matcher=None,
                            expected_matcher_schema=None,
                            request_trace_scope, request_trace_timebase):
    ...
    try:
        request_trace_scope = _request_semantics(
            request_trace_scope, "request_trace_scope")
    ...
    for label, path in (("model", trace_path), ("oracle", oracle_trace_path)):
        if not A.exists(path):
            raise RuntimeError(
                f"missing {label} raw request trace {path}; rerun the producing "
                "job with --force")
    signatures = {
    ...
    if request_path.is_file() and not force:
        ...
        if _request_cache_is_current(
                payload, request_path, signatures,
                request_trace_scope=request_trace_scope,
                request_trace_timebase=request_trace_timebase,
                expected_matcher_schema=expected_matcher_schema):
            return payload
    ...
    if result_schema != expected_matcher_schema:
        raise RuntimeError(
            f"matcher returned schema {result_schema}, expected "
            f"{expected_matcher_schema}")
    dv = result["dv"]
    if not len(dv):
        raise RuntimeError(
    ...
    if not len(full_oracle_latencies):
        raise RuntimeError(f"oracle request trace contains no reads: {oracle_trace_path}")
    mean_oracle_latency = float(full_oracle_latencies.mean())
    if not math.isfinite(mean_oracle_latency) or mean_oracle_latency <= 0:
        raise RuntimeError(
            f"oracle request trace has invalid mean read latency: {oracle_trace_path}")
    payload = {
        ...
        "mae": float(np.abs(dv).mean()) / mean_oracle_latency,
        "sgn": float(dv.mean()) / mean_oracle_latency,
        "tail": float(np.percentile(np.abs(dv), 99)) / mean_oracle_latency,
    ...
    try:
        ...
        raise RuntimeError(
            f"matcher produced an invalid request metric payload for "
            f"{trace_path}: {exc}") from exc
...
def aggregate_request_metrics(requests):
    ...
    required = {
        "mae", "sgn", "tail", "cov_o", "cov_m", "match_mode",
        "stable_cov_o", "stable_cov_m",
    }
    ...
    return {
        ...
        "worst_mae": maes[worst_mae], "worst_mae_wl": worst_mae,
        "mean_abs_sgn": sum(abs(value) for value in sgns.values()) / len(sgns),
        "worst_abs_sgn": abs(sgns[worst_sgn]),
        "worst_sgn": sgns[worst_sgn], "worst_sgn_wl": worst_sgn,
        "worst_tail": tails[worst_tail], "worst_tail_wl": worst_tail,
        "min_cov_o": cov_o[min_cov_o], "min_cov_o_wl": min_cov_o,
        "min_cov_m": cov_m[min_cov_m], "min_cov_m_wl": min_cov_m,
        ...
        "match_mode_by_workload": {
            key: value["match_mode"] for key, value in requests.items()},
    }
