...
class ServiceEnvelope final : public Model {
  ...
  struct Bank { RecentRow recent[8]; long long row = -1; long long class_row[2] = {-1,-1}; double ready = -1e30; double read_run_work = 0, read_streak_work = 0; double last_arrival=-1e30, last_row_cost=0; int last_kind = -1; };
 ...
 public:
  ...
  Cycle predict(Access a, Cycle now) override {
    ...
    // A buffered write can separate two admissions to a retained read row.
    // With more than one row cycle of bank demand, admission order becomes
    // weak evidence that the write row has displaced the read batch. Blend
    // the intrinsic startup toward read-row reuse in that queued regime.
    // This changes this admission's service estimate, never an earlier result.
    if(kind==0 && !hit && class_hit && b.last_kind==1) {
      double retained=std::clamp((backlog-row_cost)/rc,0.0,1.0);
      row_cost*=1.0-retained;
    }
    ...
    // A sustained latest-row read run supplies evidence for column service
    // overlapping other banks' preparation. Count only admitted column work:
    // elapsed idle time and shared waiting do not strengthen this evidence.
    b.read_streak_work=(kind==0 && b.last_kind==0 && hit)
      ? std::min(rc,b.read_streak_work+ccd) : 0;
    ...
    // Preserve conservative overlap for immediately ready reads in a
    // write-free busy period. A sustained same-row read run also supports
    // overlap once its admitted column demand spans a row cycle. Intervening
    // writes or row changes reset that evidence. Bank recovery and transfer
    // work (including the write-turnaround tax) still bound the prediction.
    // This is historical locality confidence, not an allocation of bus slots.
    double shared=(kind==0 && hit &&
                   ((ready<=now && !c.mixed) || b.read_streak_work>=rc))
