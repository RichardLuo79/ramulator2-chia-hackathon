...
from tools.chia_loop.framework.transport import CLI, SYSTEM_READS, relay
...
def test_fixed_relay_keeps_native_body_and_rejects_other_destinations(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("tools.chia_loop.framework.transport.httpx.stream", upstream)
...
def test_real_process_boundary_and_installed_cli_start_without_provider(tmp_path, kind):
    ...
    prefix = ["/usr/bin/python3", "-m", "tools.chia_loop.framework.native_process", str(path)]
