...
from tools.chia_loop.framework import build
...
def test_frontend_preparation_reuses_only_a_verified_capture(tmp_path, monkeypatch, existing):
    ...
    from tools.chia_loop.framework import inputs
...
def test_invalid_cpu_reservations_fail_before_staging(tmp_path, cpus):
...
def test_native_lease_survives_worker_exit_without_reaching_generated_code(tmp_path):
    ...
    from tools.chia_loop.recovery import OperationalPause, exclusive_lock

    sandbox = Path(__file__).resolve().parents[2] / "tools/chia_loop/sandbox.py"
