...
def test_native_capture_cannot_follow_an_agent_planted_file_alias(tmp_path, kind, alias):
    ...
    from tools.chia_loop.framework.config import ClaudeBackend, CodexBackend
...
def test_shared_binding_rejects_implicit_native_transports(tmp_path):
    from tools.chia_loop.framework.config import ClaudeBackend, CodexBackend, VertexBackend
    from tools.chia_loop.framework.model_sessions import create_session
...
def test_common_settings_and_source_bound_native_checkpoint(tmp_path, monkeypatch, kind):
    from tools.chia_loop.framework.config import ClaudeBackend, CodexBackend, VertexBackend
    from tools.chia_loop.framework.model_sessions import NativeCheckpoint, create_session
...
def test_native_binding_rejects_unreviewed_adapter_source(tmp_path, monkeypatch):
    ...
    from tools.chia_loop.framework.model_sessions import upstream_identity
