"""The same campaign through three actual CHIA adapters, with inert transports.
...
from tools.chia_loop.framework import agent as agent_module
from tools.chia_loop.framework.agent import TOOL_TOKEN_ENV, NativeAgent, NativeSessions, cli_usage
from tools.chia_loop.framework.build import MODEL_API_HEADER
from tools.chia_loop.framework.campaign import Campaign
from tools.chia_loop.framework.config import CampaignConfig
from tools.chia_loop.framework.model_sessions import create_session
...
def test_retry_retains_failed_native_state_and_accounting(tmp_path, failure):
    ...
    from tools.chia_loop.framework.config import CodexBackend
    from tools.chia_loop.framework.records import TransientFailure
...
def test_missing_sdk_reply_is_unknown_usage_not_zero(tmp_path):
    ...
    from tools.chia_loop.framework.config import VertexBackend
    ...
    agent.vertex_event({"kind": "request", "payload": {"fixture": True}}, None)
    agent.vertex_event({"kind": "request_error", "payload": {"message": "lost reply"}}, None)
