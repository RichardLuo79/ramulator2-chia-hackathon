...
def test_gddr7_resolver_derives_preset_owned_rck_timings():
    ...
    assert timing["nRCKSP2ST"] == 8


def test_gddr7_explicit_dependent_rck_timing_overrides_are_honored():
    dram = ramulator.dram.GDDR7(
        org_preset="GDDR7_16Gb_x8",
        timing_preset="GDDR7_28000_PAM3",
        nRCKST2SP=19,
...
def test_gddr7_test_preset_sets_sensitive_nrrd():
    dut = make_dut()

    assert dut.timings["nRRD"] == 4


def test_gddr7_org_preset_models_one_channel():
    dram = ramulator.dram.GDDR7(
        org_preset="GDDR7_16Gb_x8",
        timing_preset="GDDR7_28000_PAM3",
    )
    ...
    assert cfg["org"]["dq"] == 8
    assert cfg["org"]["count"][0] == 1
    assert cfg["org"]["count"][1] == 16
    assert cfg["channel_width"] == 8
    assert type(dram).internal_prefetch_size * cfg["channel_width"] // 8 == 32
...
def test_gddr7_start_with_read_rckstop_timing():
    ...
    t_next_read = max(
        t_rd + first_cycle_gap("RD", "RD", dut.timings["nCCD"]),
        t_stop + first_cycle_gap("RCKSTOP", "RD", dut.timings["nRCKSP2ST"]),
    )
    dut.assert_earliest_ready_at("RD", a, t_next_read)


def test_gddr7_start_with_rckstrt_command_timing():
    dut = make_dut()
    a = addr(dut)
    rck = all_bank_addr(dut)
    ...
    t_rd = max(
        first_cycle_gap("ACT", "RD", dut.timings["nRCDRD"]),
        4 + first_cycle_gap("RCKSTRT", "RD", dut.timings["nRCKSTRT2RD"]),
    )
    dut.assert_earliest_ready_at("RD", a, t_rd)
    dut.issue("RD", a, t_rd)
