...
from ramulator_chia.framework.export import (
    export_campaign,
...
from ramulator_chia.framework.tool_server import phase_tools, result_value
from ramulator_chia.framework.workspace import Workspace
from ramulator_chia.eval import config
...
class ScriptedSession:
    ...
    def work(self, phase, inputs, methods):
        view = self.view
        staged = bool(view.research.configuration.experiment.stages)
        if phase == "explore":
            ...
            workload = view.research.configuration.experiment.evaluation.primary.training[0]
            if "inspect_input" in methods:
                page = methods["inspect_input"](workload, offset=2_000_000, instructions=3)
                assert len(page["instructions"]) == 3
            ...
            if staged:
                methods["read"]("references/incumbent/model.cpp")
                methods["read"]("references/comparison.json")
                return {"text": canonical_json({
                    "decision": "keep", "rationale": "Only a comment changed; no accuracy benefit.",
                    "improvements": [], "accepted_regressions": [], "contract_findings": [],
                    "contract_violation": False, "uncertainty": [],
                }), "fixture": True}
        elif phase == "reflect":
            if staged:
                validation = inputs["scores"]["validation"]
                assert validation["available"]
                for group in validation["groups"].values():
                    assert all(name.startswith("val") for name in group["cases"])
                methods["write"]("notes/summary.md", "# Staged scripted fixture\n\n"
                                 "Comment-only candidate; no model optimization.\n\n"
                                 + canonical_json(inputs) + "\n")
                return {"text": "Staged summary saved.", "fixture": True}
            if view.research.configuration.experiment.evaluation.validation:
                validation = inputs["validation"]
                assert all(name.startswith("val") for name in validation["workloads"])
                for scores in (validation["aggregate"], *validation["workloads"].values()):
                    assert set(scores) == {"core_error_pct", "request_mae_over_L"}
            methods["write"](
                "notes/summary.md",
...
@contextmanager
def scripted_session(research, iteration, role, candidate, history, **options):
    yield ScriptedSession(Workspace(research, iteration, role, candidate, history, **options))
...
def run(
    ...
    if archive_output is not None:
        ...
        if configuration.experiment.evaluation.champsim:
            frontends.add("champsim")
        ...
        if transfer.gem5 is not None:
            ...
            from ramulator_chia.eval.gem5.build_suite import source_recipes
