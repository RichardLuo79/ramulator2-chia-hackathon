...
from ramulator_chia.framework.records import (
    CampaignState,
    RecordConflict,
    TransientFailure,
    UnresolvedStep,
...
def test_operator_retry_extension_is_one_step_and_one_attempt(tmp_path):
    state = CampaignState(tmp_path / "campaign.sqlite")
    settings = {"maximum_attempts": 1, "retry_delay_seconds": 0}
    inputs = {"round": 11}

    def fail():
        raise TransientFailure("provider rate limit")

    for key in ("explore", "other"):
        with pytest.raises(TransientFailure):
            state.step(key, inputs, fail, **settings)
    before = state._query("SELECT * FROM attempts WHERE step='explore'")
    evidence = {"checkpoint_sha256": "a" * 64, "reason": "operator authorized restart"}
    with pytest.raises(RecordConflict):
        state.authorize_retry_extension("explore", "wrong", evidence)
    with pytest.raises(RecordConflict):
        state.authorize_retry_extension("explore", digest_json(inputs), {})
    state.authorize_retry_extension("explore", digest_json(inputs), evidence)
    assert state._query("SELECT * FROM attempts WHERE step='explore'") == before
    restarted = CampaignState(tmp_path / "campaign.sqlite")
    with pytest.raises(UnresolvedStep, match="attempt limit"):
        restarted.step("other", inputs, fail, **settings)
    with pytest.raises(TransientFailure):
        restarted.step("explore", inputs, fail, **settings)
    with pytest.raises(UnresolvedStep, match="attempt limit"):
        restarted.step("explore", inputs, fail, **settings)
    assert len(restarted._query("SELECT * FROM attempts WHERE step='explore'")) == 2


def test_operator_retry_extension_cannot_override_ambiguous_or_complete_step(tmp_path):
    state = CampaignState(tmp_path / "campaign.sqlite")
    options = {"maximum_attempts": 1, "retry_delay_seconds": 0}
    with pytest.raises(RuntimeError):
        state.step("unknown", {}, lambda: (_ for _ in ()).throw(RuntimeError("unknown")), **options)
    state.step("done", {}, lambda: {"success": True}, **options)
    for key in ("unknown", "done"):
        with pytest.raises(RecordConflict):
            state.authorize_retry_extension(key, digest_json({}), {
                "checkpoint_sha256": "a" * 64, "reason": "operator authorized restart",
            })


def test_verified_native_continuation_keeps_interrupted_attempt(tmp_path):
    state = CampaignState(tmp_path / "campaign.sqlite")
    inputs = {"round": 11}
    with pytest.raises(RuntimeError, match="interrupted"):
        state.step("explore", inputs, lambda: (_ for _ in ()).throw(RuntimeError("interrupted")),
                   maximum_attempts=3, retry_delay_seconds=0)
    with pytest.raises(RecordConflict):
        state.authorize_native_continuation("explore", "wrong", {"checkpoint_sha256": "a" * 64})
    with pytest.raises(RecordConflict):
        state.authorize_native_continuation("explore", digest_json(inputs), {})
    state.authorize_native_continuation("explore", digest_json(inputs), {"checkpoint_sha256": "a" * 64})
    assert "interrupted" in state.get("continuation:explore:1")["original_error"]
    assert state.completed_step("explore") is None
    assert state.step("explore", inputs, lambda: {"continued": True}, maximum_attempts=3,
                      retry_delay_seconds=0) == {"continued": True}
    assert [r["status"] for r in state._query("SELECT status FROM attempts ORDER BY number")] == ["failed", "complete"]
    with pytest.raises(RecordConflict):
        state.authorize_native_continuation("explore", digest_json(inputs), {"checkpoint_sha256": "a" * 64})
...
def test_retry_delay_is_applied_only_between_attempts(tmp_path, monkeypatch):
    from ramulator_chia.framework import records
