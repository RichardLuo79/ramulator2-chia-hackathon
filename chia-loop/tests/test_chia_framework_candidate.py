...
from ramulator_chia.framework import candidate as implementation
from ramulator_chia.framework.build import MODEL_API_HEADER, MODEL_INTERFACE, file_sha256
from ramulator_chia.framework.candidate import (
    RECIPE,
    BuildFailed,
    BuildLimits,
    compile_snapshot,
...
from ramulator_chia.framework.snapshots import InvalidSnapshot, ModelFiles, publish_bytes, snapshot
...
def runtime(tmp_path):
    ...
    api, recipe = REPO / MODEL_API_HEADER, NATIVE / 'model/CMakeLists.txt'
...
def seed():
    return (NATIVE / "model/seed.cpp").read_text()
