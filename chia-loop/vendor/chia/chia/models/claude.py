...
class ClaudeCodeQueryResult(QueryResult):
    ...
    native_events: Optional[list] = None
...
class ClaudeCodeLLM(LLMCallBase):
    ...
    def __init__(
        ...
        max_tool_iterations: Optional[int] = 100,
        ...
        self.generation_config = deepcopy(generation_config or {})
        if set(self.generation_config) & {"model", "messages", "system", "tools", "max_tokens"}:
            raise ValueError("generation settings cannot replace model, history or tools")
    ...
    def prompt_once(self, user_message: str, tools=None) -> ClaudeCodeQueryResult:
        ...
        if self.backend == "api":
            ...
            def capture(result):
                ...
                result.native_events = deepcopy(self._native_events)

            return single_attempt(
                lambda: self._run_api(user_message, tools or []),
                lambda: None, capture, ClaudeCodeQueryResult,
            )
    ...
    def session_state(self) -> dict:
        """Native Messages blocks, including opaque thinking and compaction."""
        return deepcopy({"messages": self._messages,
                         "open_user_message": self._open_user_message,
                         "pending_tools": self._pending_tools})

    def restore_session(self, state: dict) -> None:
        if self.backend != "api" or not self._resume_session or set(state) != {
            "messages", "open_user_message", "pending_tools"
        }:
            raise ValueError("unexpected Anthropic continuation")
        if not isinstance(state["messages"], list) or any(
            not isinstance(message, dict) for message in state["messages"]
        ) or type(state["pending_tools"]) is not bool or (
            state["open_user_message"] is not None and not isinstance(state["open_user_message"], str)
        ):
            raise ValueError("invalid Anthropic checkpoint")
        self._messages = deepcopy(state["messages"])
    ...
    @staticmethod
    def _api_json(value):
        # SDK model_dump retains provider extensions and opaque signatures.
        # The fallback supports CHIA's existing lightweight offline fixtures.
        if hasattr(value, "model_dump"):
            return value.model_dump(mode="json", exclude_none=True)
        if isinstance(value, dict):
            return {k: ClaudeCodeLLM._api_json(v) for k, v in value.items()}
        if isinstance(value, (list, tuple)):
            return [ClaudeCodeLLM._api_json(v) for v in value]
        if hasattr(value, "__dict__"):
            return ClaudeCodeLLM._api_json(vars(value))
        return value

    async def _api_response(self, client, kwargs):
        """One provider request, never a deadline on time spent running tools."""
        import asyncio

        endpoint = client.beta.messages if kwargs.get("betas") else client.messages
        stream = None
        ...
        async def request():
            ...
            async with endpoint.stream(**kwargs) as stream:
                ...
                if response.stop_reason is None:
                    raise ServerError(self._get_node_id(), raw_message="provider stream ended before message completion")
                return response

        try:
            return await asyncio.wait_for(request(), timeout=self.timeout_seconds)
        except BaseException:
            # Preserve an interrupted stream without turning an incomplete
            # assistant response into valid conversation history or tool calls.
            if stream is not None:
                try:
                    partial = stream.current_message_snapshot
                except (RuntimeError, AttributeError):
                    partial = None
                if partial is not None:
                    self._native_event("partial_response", self._api_json(partial))
            raise
    ...
    async def _run_api_async(
        ...
        if self._resume_session and self._pending_tools:
            raise RuntimeError("Anthropic session has unresolved tools; reconcile before continuing")
        if self._resume_session and self._open_user_message not in (None, user_message):
            raise ValueError("cannot replace an unfinished Anthropic turn with a different prompt")
        client = self.api_client_factory() if self.api_client_factory is not None else (
        ...
        async with AsyncExitStack() as stack:
            ...
            for tool in tools or []:
                ...
                if self.tool_http_client_factory is not None:
                    http_options["http_client"] = await stack.enter_async_context(
                        self.tool_http_client_factory(tool)
                    )
                transport = await stack.enter_async_context(streamable_http_client(url, **http_options))
                ...
                for fn in listed.tools:
                    ...
                    if api_name in dispatch:
                        raise ValueError("MCP tool names collide after API normalization")
            ...
            messages = self._messages if self._resume_session else []
            ...
            while self.max_tool_iterations is None or meta["num_turns"] < self.max_tool_iterations:
                kwargs: dict = {
                    "model": self.model,
                ...
                self._native_event("request", kwargs)
                try:
                    resp = await self._api_response(client, kwargs)
                except BaseException as exc:
                    self._native_event("request_error", {"type": type(exc).__name__})
                ...
                # Do not splice or rewrite signed thinking around compaction.
                # The provider handles earlier blocks; original history stays
                # archived in the native request/response events.
                content = self._api_json(resp.content)
                messages.append({"role": "assistant", "content": content})
                self._pending_tools = bool(tool_uses)
                self._native_event("response", self._api_json(resp))

                if any(b.get("type") == "compaction" and not b.get("content") for b in content):
                    raise RuntimeError("provider returned an empty compaction; continuation stopped")
                ...
                if resp.stop_reason in {"pause_turn", "compaction"} and not tool_uses:
                    continue
                if resp.stop_reason != "tool_use" or not tool_uses:
                    if resp.stop_reason not in {"end_turn", "stop_sequence"} or tool_uses:
                        raise RuntimeError(f"unexpected Anthropic stop reason: {resp.stop_reason}")
                    self._open_user_message = None
                    self._native_event("turn_complete", {"stop_reason": resp.stop_reason})
                    break
                ...
                for tu in tool_uses:
                    self._native_event("tool_request", self._api_json(tu))
                    session, fn_name = dispatch.get(tu.name, (None, None))
                    if session is None:
                        result_text = f"Unknown tool: {tu.name}"
                        is_error = True
                    else:
                        # MCP isError is a settled tool result. A transport
                        # exception can hide a committed effect: stop, retaining
                        # pending_tools rather than inventing an error result.
                        mcp_result = await session.call_tool(fn_name, tu.input or {})
                    ...
                    self._pending_tools = len(tool_results) != len(tool_uses)
                    self._native_event("tool_response", tool_results[-1])

                self._native_event("tools_complete", {})
            else:
                raise RuntimeError("Anthropic tool iteration limit reached")
    ...
    def _translate_api_error(self, exc) -> Optional[Exception]:
        ...
        if isinstance(exc, TimeoutError):
            return ServerError(node_id=node_id, raw_message="provider request timeout")
