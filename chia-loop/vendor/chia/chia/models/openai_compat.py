...
@dataclass
class OpenAICompatQueryResult(QueryResult):
...
class OpenAICompatLLM(LLMCallBase):
    ...
    def __init__(
        ...
        tool_http_client_factory: Optional[Callable] = None,
        ...
        # (directly or via a **kwargs subclass like VLLMLLM) warns it's ignored.
        ...
        if set(self.generation_config) - {"reasoning_effort", "extra_body"}:
            raise ValueError("generation settings cannot override messages, model or tools")
        if set(self.generation_config.get("extra_body", {})) - {"thinking"}:
            raise ValueError("extra_body is reserved for the provider thinking setting")
    ...
    def prompt_once(self, user_message: str, tools=None) -> OpenAICompatQueryResult:
        """One continuous tool turn; the caller owns retries and durable evidence."""
        self._native_events = []
        self._last_metadata = {}
        ...
        return single_attempt(
            lambda: self._run_openai(user_message, tools or []),
            lambda: None, capture, OpenAICompatQueryResult,
        )
    ...
    def restore_session(self, state: dict) -> None:
        if not self._resume_session or set(state) != {"messages", "open_user_message", "pending_tools"}:
            raise ValueError("unexpected chat-completions continuation")
        if not isinstance(state["messages"], list) or any(
            ...
            raise ValueError("invalid chat-completions checkpoint")
    ...
    async def _run_openai_async(
        ...
        if self._resume_session and self._pending_tools:
            raise RuntimeError("chat session has unresolved tools; reconcile before continuing")
        if self._resume_session and self._open_user_message not in (None, user_message):
            raise ValueError("cannot replace an unfinished chat turn with a different prompt")
        ...
        async with AsyncExitStack() as stack:
            ...
            while self.max_tool_iterations is None or meta["num_turns"] < self.max_tool_iterations:
                ...
                if self.context_compactor is not None:
                    await self.context_compactor(self, client, messages, kwargs)
                ...
                if not resp.choices:
                    ...
                    raise UnknownOpenAIError(self._get_node_id(), raw_message="response has no choices")
                ...
                # The SDK retains provider extras such as reasoning_content.
                # Reconstructing only content/tool_calls loses reasoning context.
                messages.append(msg.model_dump(mode="json", exclude_none=True))
                self._pending_tools = bool(tool_calls)
                self._native_event("response", resp.model_dump(mode="json", exclude_none=True))
                ...
                if not tool_calls:
                    if finish_reason != "stop":
                        raise UnknownOpenAIError(self._get_node_id(), raw_message=f"unexpected finish_reason: {finish_reason}")
                    self._open_user_message = None
                    self._native_event("turn_complete", {"finish_reason": finish_reason})
                    break

                # --- Execute each requested tool over its MCP server ---
                for tc in tool_calls:
                    self._native_event("tool_request", tc.model_dump(mode="json", exclude_none=True))
                    ...
                    self._native_event("tool_response", {"tool_call_id": tc.id, "content": result_text, "isError": is_error})
                self._pending_tools = False
                self._native_event("tools_complete", {})
            else:
                raise UnknownOpenAIError(self._get_node_id(), raw_message="tool iteration limit reached")
        ...
        return OpenAICompatQueryResult(
            result=final_text,
            returncode=0,
            stderr="",
