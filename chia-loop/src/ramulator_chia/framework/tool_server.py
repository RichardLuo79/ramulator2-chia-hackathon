...
JOBS = frozenset({"build", "evaluate_training", "synthetic", "open_loop", "inspect_training", "inspect_input"})
...
class WorkspaceTool(AuthenticatedJobTool):
    ...
    def background(self, method):
        ...
        if method.__name__ in {"open_loop", "synthetic"}:
            resources = method.__self__.research.resources
            ...
            start.__doc__ += (
                f" Each diagnostic simulator has a {timeout}-second wall deadline after launch."
                " Timeout preserves partial evidence but supplies no accuracy score;"
                " it is not proof of deadlock. No automatic retry."
            )
        return start
