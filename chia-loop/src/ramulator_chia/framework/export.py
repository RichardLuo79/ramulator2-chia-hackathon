...
HARNESS_TREES = (
    "chia-loop/src/ramulator_chia",
    
    "ramulator/python/ramulator",
    "ramulator/resources/champsim_bridge",
    "ramulator/resources/gem5_wrappers",
)
HARNESS_FILES = ('chia-loop/pyproject.toml', 'chia-loop/requirements.txt', 'LICENSE')
SOURCE_SUFFIXES = {
    ".py",
    ".c",
    ".cc",
...
def capture_sources(repo: Path, runtime: Path, destination: Path) -> dict:
    ...
    repo = repo.parent if repo.name == "ramulator" else repo
...
def export_campaign(campaign, research, output: Path, *, external_recipes: dict | None = None):
    ...
    with exclusive_lock(root / "campaign.lock"):
        ...
        if (root / "context-compaction.json").is_file():
            add(root / "context-compaction.json", "campaign/context-compaction.json")
        for name, host in research.hosts.items():
            add(host.root / "host.json", f"campaign/frontends/{name.replace(':', '/')}/host.json")
        for directory in (
            "candidates",
            "summaries",
            "events",
        ...
        # Several topology-specific builds share one upstream source recipe.
        # Keep every evaluated build identity, without inventing a new frontend
        # (or bundling its executable) for each core count.
        frontends = {name.split(":", 1)[0] for name in hosts}
        validate_recipes(external_recipes, frontends)
        references = {}
        for name, recipe in external_recipes.items():
            metadata = recipe["metadata"]
            builds = {key: host for key, host in hosts.items() if key.split(":", 1)[0] == name}
            references[name] = dict(metadata)
            if name in hosts:
                references[name]["evaluated_host"] = hosts[name]  # Legacy export field.
            if any(":" in key for key in builds):
                references[name]["evaluated_builds"] = builds
