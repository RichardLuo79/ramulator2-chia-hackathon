...
from ramulator_chia.core import atomic_write_json

from .identity import file_sha256 as file_sha256
from ramulator_chia.layout import build_source, build_key

SOURCE_TREES = ("src", "ext/fmt", "ext/yaml-cpp", "ext/mess", "ext/sniper_wmg1")
SOURCE_FILES = (
    "CMakeLists.txt",
    "tools/chia_loop/CMakeLists.txt",
    "tools/chia_loop/isolated_sim.cpp",
    "tools/chia_loop/dram_speed_inputs.cpp",
...
def _source_files(repo: Path):
    ...
    for relative in SOURCE_FILES:
        yield build_source(repo, relative)
...
def stage_source(repo: Path, destination: Path) -> dict[str, str]:
    ...
    for source in paths:
        ...
        relative = build_key(repo, source)
...
def prepare_runtime(
    ...
    if type(cpus) is not int or not 1 <= cpus <= available:
        raise ValueError(f"build requires 1 to {available} reserved CPUs on this host")
    ...
    configure = run_build_command(
        [
            ...
            f"-DRAMULATOR_CHIA_SOURCE_DIR={source / 'tools/chia_loop'}",
...
def main(argv=None):
    ...
    parser.add_argument("--repo", type=Path, default=__import__("ramulator_chia.layout", fromlist=["RAMULATOR"]).RAMULATOR)
    parser.add_argument("--output", type=Path, required=True, help="new runtime directory")
    parser.add_argument("--workers", type=int, choices=range(1, len(os.sched_getaffinity(0)) + 1), default=6)
