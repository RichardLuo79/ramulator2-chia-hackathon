...
def oracle_population(path, controller_stats, *, require_complete_admissions=True):
    ...
    if type(require_complete_admissions) is not bool:
        raise ValueError("admission completeness must be an explicit boolean")
    if require_complete_admissions and any(
