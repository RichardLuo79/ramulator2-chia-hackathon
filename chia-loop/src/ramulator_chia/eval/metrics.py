...
def paired_error_statistics(delta, oracle_mean_latency, *, include_tails=False):
    ...
    if include_tails:
        ...
        p999 = float(np.percentile(absolute, 99.9, method=PERCENTILE_METHOD))
        total = float(absolute.sum())
        result.update(paired_p999_cycles=p999, p999_over_L=p999 / scale)
        result["tail_thresholds"] = {}
        for multiple in (1, 5):
            selected = absolute > multiple * scale
            result["tail_thresholds"][f"{multiple}L"] = {
                "count": int(selected.sum()),
                "fraction": float(selected.mean()),
                "absolute_error_share": float(absolute[selected].sum()) / total if total else 0.0,
            }
    return result
...
def validate_request_payload(
    ...
    *,
    ...
    if payload.get("trusted_pair_policy") == "champsim_exact_physical_filter":
        logical = request_count(payload.get("stable_logical_pairs"), "stable_logical_pairs")
        mismatches = request_count(payload.get("address_mismatch_pairs"), "address_mismatch_pairs")
        physical = request_count(payload.get("physical_consistent_pairs"), "physical_consistent_pairs")
        if (
            mode != "stable_id"
            or logical > min(eligible_o, eligible_m)
            or logical != matched + mismatches
            or physical != matched
        ):
            raise ValueError("ChampSim logical/physical pair counts are inconsistent")
        _require_ratio(
            request_coverage(payload.get("physical_consistency_rate"), "physical_consistency_rate"),
            matched, logical, "physical_consistency_rate",
        )
    return payload
...
def request_payload_from_match(
    ...
    *,
    ...
    payload = {
        ...
        **paired_error_statistics(dv, mean_oracle_latency, include_tails=include_tails),
