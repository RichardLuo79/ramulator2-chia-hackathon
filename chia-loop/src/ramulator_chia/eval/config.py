...
OUT = pathlib.Path(os.environ.get("EVAL_OUT", REPO.parent / ".work/evaluations"))
TRACES = pathlib.Path(os.environ.get("RAMULATOR_TRACES", str(__import__("ramulator_chia.layout", fromlist=["DATA"]).DATA / "traces")))
OPT_BUILD = pathlib.Path(
    os.environ.get("RAMULATOR_OPT_BUILD", REPO.parent / ".work/runtime/runtime")
)
...
MESS_CURVES = {
    "DDR5": pathlib.Path(__file__).resolve().parent / "calibration" / "mess_DDR5.txt",
}
...
# ── gem5 ─────────────────────────────────────────────────────────────────
GEM5_BIN = os.environ.get("GEM5_BIN", str(REPO.parent / ".work/gem5/build/X86/gem5.opt"))
...
# ── ChampSim ─────────────────────────────────────────────────────────────
CHAMPSIM_DIR = pathlib.Path(os.environ.get("CHAMPSIM_DIR", str(REPO.parent / ".work/champsim")))
