...
// memory controller as one aggregate server. zsim scales ZERO-LOAD LATENCY
// by 1 + rho/(2*(1-rho)); the bandwidth service time determines rho, not
// the scale of the added delay. Access counts (not rates) are smoothed at
// demand-driven phase boundaries, including elapsed empty phases.
// Our phase clock is the controller clock, not zsim's frontend barrier.
// See doc/baseline_source_audit.md for the pinned source and adaptations.
class MD1Controller final : public IController, public Implementation {
 ...
 public:
  void init() override {
    ...
    RAMULATOR_PARSE_PARAM(m_smoothing, double, "smoothing").default_val(0.5);
    ...
    if (m_phase_ticks <= 0 || !(m_smoothing > 0 && m_smoothing <= 1)) {
      throw std::runtime_error("MD1: phase_ticks and smoothing must be positive; smoothing <= 1");
    }
    m_latency = m_rl;
  }
 ...
 private:
  ...
  void roll_phase(Clk_t t) {
    const Clk_t phase = t / m_phase_ticks;
    const Clk_t elapsed = (phase - m_last_phase) * m_phase_ticks;
    if (elapsed < 10000) return;  // zsim's minimum update interval.
    m_smoothed_accesses = m_smoothing * m_phase_accesses +
                          (1.0 - m_smoothing) * m_smoothed_accesses;
    double rho = (m_smoothed_accesses / static_cast<double>(elapsed)) * m_burst;
    if (rho > 0.95) {
      rho = 0.95;
      s_clamped_loads++;
    }
    m_latency = static_cast<Clk_t>((1.0 + 0.5 * rho / (1.0 - rho)) * m_rl);
    m_phase_accesses = 0;
    m_last_phase = phase;
  }
  ...
  Clk_t m_clk = 0;
  Clk_t m_last_phase = 0;
  Clk_t m_latency = 0;
  size_t m_phase_accesses = 0;
  double m_smoothed_accesses = 0.0;
